...
MODEL_PATH = PROJECT_DIR / "models" / "house_price_model.joblib"
...
def load_training_data(data_path: Path = DATA_PATH) -> pd.DataFrame:
    if not data_path.exists():
        ...
        print("Replace it with Kaggle House Prices train.csv for the final model.")
    return pd.read_csv(data_path)


def pick_target(df: pd.DataFrame) -> str:
    for target in ("SalePrice", "sale_price", "price", "Price"):
        if target in df.columns:
            return target
    raise ValueError("No target column found. Expected SalePrice, sale_price, price, or Price.")
...
def train(data_path: Path = DATA_PATH) -> tuple[Pipeline, dict[str, object]]:
